namespace Costura.Fody.Tests
{
    public class ResourceEmbedderTests
    {
        ...
        [Test]
        [Arguments("shortpath.dll", $"{CacheRoot}\\{DummyShaChecksum}.shortpath.dll.compressed")]
        [Arguments("runtimes.unix.lib.netcoreapp2.1.microsoft.data.sqlclient.dll", $"{CacheRoot}\\{DummyShaChecksum}.1.compressed")]
        public async Task TheGetCacheFileMethod(string resourceName, string expectedOutput)
        {
            // Just dummy values
